...
if ( isset( $data['otp'] ) ) {
	...
	if ( $result->num_rows > 0 ) {
		...
		while ( $row = $result->fetch_assoc() ) {
			// Check if the OTP entered matches the one sent to the user
			if ( $row['otp'] == $otp_post ) {
				...
				if ( $conn->query( $deleteColumnQuery ) === true ) {
					...
					echo json_encode( [
						'type'     => 'success',
						'title'    => 'Success',
						'text'     => 'OTP verified successfully and OTP column deleted.',
						'redirect' => 'change-password.php' // Redirect to change password page
					] );
				} else {
